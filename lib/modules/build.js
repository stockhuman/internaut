...
class Generator {
  ...
  all () {
    ...
    this.styles()
    this.scripts()
    this.scripts('./site/scripts/home.js', './build/assets/js/home.min.js') // WARN: hardcoded
  }
    ...
    const ditherfolder = async folder => {
      ...
      await Promise.all(list.map(async file => {
        ...
        if (filepath.ext.match(/.(jpg|jpeg|png)$/i)) {
          let img = new Image(filepath.full)
          try {
            await img.resize()
            await img.dither()
            await img.save({
  ...
  async assets (asset = null) {
    ...
    if (asset) {
      ...
      if (isImage(asset)) {
        ...
        fse.copyFileSync(`${this.srcPath}/assets/${formattedpath}`,
  ...
  styles (entry, dest, options = { outputStyle: 'compressed' }) {
    ...
    const result = sass.renderSync({ file: entry, ...options })
