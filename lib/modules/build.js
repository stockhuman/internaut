...
class Generator {
  ...
  async scripts (entry, dest) {
    ...
    let result = babel.transform(entry, { presets: ['env'] }).code
    ...
    //   input: entry,
    //   plugins: [
    //     babel({ 'presets': [['@babel/env', { "modules": false }]] })
    //   ]
    // })
    // .then(bundle => bundle.generate({ format: 'iife' }))
    // .then(result => fse.writeFileSync(dest, result.output[0].code))
  }
}
